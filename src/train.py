...
def process_data(data_path):
    df = pd.read_csv(data_path, encoding="latin-1", error_bad_lines=False)
...
if __name__ == "__main__":
    ...
    #saving metadata
    joblib.dump(meta_data,"../input/data/meta.bin")
    ...
    train_dataset = my_dataset.EntityDataset(texts=train_sentences, pos=train_pos, tags=train_tag)

    train_data_loader = torch.utils.data.DataLoader(train_dataset, batch_size=config.TRAIN_BATCH_SIZE, num_workers=4)

    valid_dataset =my_dataset.EntityDataset(
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, warmup_steps=0, t_total=num_train_steps
    )
